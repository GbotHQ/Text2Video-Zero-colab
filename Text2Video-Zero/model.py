...
class Model:
    ...
    def process_controlnet_canny_db(
        self,
        db_path,
        video_path,
        prompt,
        ...
        if "db_path" not in self.states or db_path != self.states["db_path"]:
            ...
            self.set_model(
                ModelType.ControlNetCannyDB, model_id=db_path, controlnet=controlnet
            )
            self.pipe.scheduler = DDIMScheduler.from_config(self.pipe.scheduler.config)
    ...
    def process_pix2pix(
        ...
        if self.model_type != ModelType.Pix2Pix_Video:
            self.set_model(
                ModelType.Pix2Pix_Video, model_id="timbrooks/instruct-pix2pix"
            )
