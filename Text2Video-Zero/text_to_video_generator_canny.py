...
class TextToVideoCanny:
    def __init__(
        ...
        self.model = (
            create_model("./models/cldm_v15.yaml").cpu()
            if use_cf_attn
            else create_model("./models/cldm_v15_no_cf_attn.yaml").cpu()
        )
        self.model.load_state_dict(
            load_state_dict("./models/control/control_sd15_canny.pth", location="cuda")
        )
        ...
        self.neg_prompt = (
            neg
            if neg
            else "longbody, lowres, bad anatomy, bad hands, missing fingers, extra digit, fewer digits, cropped, worst quality, low quality"
        )
    ...
    def inference(
        self, prompt, motion_path="__assets__/canny_videos_correct/deer_orig.mp4"
    ):
        ...
        list_of_input_image = list_of_input_image[
            : (len(list_of_input_image) // self.n) * self.n
        ]
        chunks = [
            list_of_input_image[x : x + 8]
            for x in range(0, len(list_of_input_image), 8)
        ]
        ...
        for chunk in chunks:
            with torch.no_grad():
                ...
                for input_image in chunk:
                    control, detected_map = self.pre_process(
                        input_image, self.res, self.low, self.high
                    )
                ...
                un_cond = {
                    ...
                    "c_crossattn": [
                        self.model.get_learned_conditioning([self.neg_prompt] * self.n)
                    ],
                }
